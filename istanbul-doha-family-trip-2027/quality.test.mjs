...
test("visible comparison states the fixed Istanbul and Wednesday route gate", async () => {
  const html = await readFile(join(here, "index.html"), "utf8");
  for (const phrase of [
    "IST 7박 → DOH 3박",
    "달력상 여행은 10박",
  ...
  assert.match(html, /2027년 3월 31일은 수요일/);
  assert.match(html, /도하안을 폐기하고 두바이안/);
});
...
test("entry point reuses the shared renderer with Doha data and core", async () => {
...
test("Doha action colors retain readable text and visible control boundaries", async () => {
...
test("map, nearby, assistant and export controls keep their accessible labels", async () => {
  const html = await readFile(join(here, "index.html"), "utf8");
  assert.match(html, /id="map-canvas"[^>]+aria-label="도하 장소 지도"/);
...
test("assistant-authored Doha files contain no Unicode middle dot", async () => {
