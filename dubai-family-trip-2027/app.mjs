...
const answerKey = "dubai-family-trip-answers-v1";
const state = {
...
function renderToday() {
  ...
  if (status.mode === "planning") {
    ...
    card.innerHTML = `
      ...
      <h3>먼저, 숙소의 정원과 위치 중 무엇을 우선할지 잠급니다.</h3>
      <p>Zabeel Saray는 6성인과 3아동을 공식 수용하지만 Palm 서쪽입니다. Marsa Al Arab은 이동이 더 짧지만 6+3 수용을 서면 승인받아야 합니다.</p>
...
function mapSvg() {
  ...
  const points = [
    ...
    ...buildMapPoints(lodgingOptions.slice(0, 2).map((item) => ({ ...item, category: "숙소", duration: "기준점", energy: 1, rain: true, why: item.verdict, warning: item.cautions[0] })))
  ];
  const unique = [...new Map(points.map((item) => [item.id, item])).values()];
  return `<svg viewBox="0 0 1000 560" aria-labelledby="map-title">
    <title id="map-title">두바이 주요 장소의 상대 위치</title>
    ...
    <text class="map-label" x="32" y="55">PALM JUMEIRAH</text>
    <text class="map-label" x="740" y="92">DOWNTOWN</text>
    <text class="map-label" x="418" y="520">JUMEIRAH COAST</text>
...
function endpointFromSettings() {
  ...
  if (provided && /^https:\/\//.test(provided)) {
    try { localStorage.setItem("dubai-assistant-endpoint", provided); } catch { /* ignore */ }
    return provided;
  }
  try { return localStorage.getItem("dubai-assistant-endpoint") || ""; } catch { return ""; }
}
...
function wireUtilities() {
  $("#download-csv").addEventListener("click", () => downloadFile("dubai-family-trip-places.csv", `\uFEFF${makeCsv()}`, "text/csv;charset=utf-8"));
  $("#download-kml").addEventListener("click", () => downloadFile("dubai-family-trip-places.kml", makeKml(), "application/vnd.google-earth.kml+xml"));
