...
test("visible comparison states the physical stay and paid double-booking separately", async () => {
  ...
  assert.match(html, /2027년 목표일 실가격은 아직 확정하지 않았습니다/);
});

test("catalog labels match the bounded three-night research depth", async () => {
  const html = await readFile(join(here, "index.html"), "utf8");
  for (const phrase of ["12 HOTELS", "레지던스 6곳", "30 PLACES", "음식점 20곳과 카페 10곳"]) {
    assert.ok(html.includes(phrase), phrase);
  }
...
test("entry point reuses the shared renderer with Antalya data and core", async () => {
...
test("map, nearby, assistant and export controls keep their accessible labels", async () => {
  const html = await readFile(join(here, "index.html"), "utf8");
  assert.match(html, /id="map-canvas"[^>]+aria-label="안탈리아 장소 지도"/);
...
test("assistant-authored Antalya files contain no Unicode middle dot", async () => {
