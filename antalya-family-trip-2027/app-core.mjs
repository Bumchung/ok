...
export function weatherMode(now = new Date()) {
  ...
  if (untilArrival > 16) {
    return { live: false, reason: "실시간 예보는 안탈리아 출발 16일 전부터 확인합니다.", untilArrival };
  }
  if (untilArrival < -7) {
    return { live: false, reason: "안탈리아 체류 기간이 지났습니다.", untilArrival };
  }
...
function searchableRecords(days = itinerary) {
  ...
  const budgetRecords = list(budgetModel?.stayOptions).map((item) => {
    ...
    return {
      ...
      body: `${item.note} 안탈리아 추가 가족 총액 ${Math.round(result.landFamilyTotal).toLocaleString("ko-KR")}원, 국내선 포함 전체 추가액 ${Math.round(result.mixedOriginFamilyTotal).toLocaleString("ko-KR")}원. 실시간 결제 견적이 아닙니다.`,
      url: "#budget"
    };
  });
...
function hotelAnswer() {
  ...
  return `우선 확인할 호텔은 ${names}입니다. 3월 실내 수영장 어린이 이용 시간, 객실 4실의 연결 또는 인접 배정, 난방과 취소 조건을 서면으로 받은 뒤 결정하세요. 2027년 3월 26일부터 29일까지의 같은 조건 실가격은 아직 확정값으로 쓰지 않습니다.`;
}
...
function budgetAnswer() {
  ...
  return `표시된 금액은 이스탄불 원안에 더해지는 안탈리아 계획값입니다. 기본 시나리오의 가족 추가액은 예비비 포함 약 ${Math.round(result.mixedOriginFamilyTotal / 10_000).toLocaleString("ko-KR")}만원입니다. 국내선, 안탈리아 3박, 전용차를 분리해 보고 실제 2027년 항공과 객실이 열리면 같은 조건으로 다시 계산하세요.`;
}

const questionRules = [
  {
    terms: ["비오", "비가", "우천", "날씨"],
    answer: "비가 오면 야외 유적을 늘리지 말고 Antalya Aquarium이나 Antalya Toy Museum 가운데 한 곳만 봅니다. 현재 폐관한 Antalya Museum은 우천 대안으로 쓰지 않습니다."
  },
  {
    terms: ["박물관", "museum"],
    answer: "Antalya Museum은 현재 공식 폐관 상태이므로 본 일정과 지도에서 제외했습니다. 2027년 재개관이 공식 확인되기 전까지 대안으로 계산하지 않습니다."
  },
  {
    terms: ["perge", "페르게", "aspendos", "아스펜도스"],
    answer: "Perge와 Aspendos를 같은 날 묶지 않는 편이 낫습니다. 아이 셋과 유적 두 곳을 이어 보면 차량 시간과 야외 노출이 늘어납니다. 이번 3박에서는 Perge 90분과 가까운 한 장면만 남기세요."
  },
  {
  ...
  {
    terms: ["해변", "바다", "수영", "해수욕"],
    answer: "3월 안탈리아는 해수욕 여행이 아닙니다. 해안 산책과 바다 풍경은 좋지만 평균 해수온 약 16°C를 전제로 실내 수영장과 우천 대안을 먼저 확인하세요."
  },
  {
    terms: ["항공", "비행", "공항", "saw", "ist"],
    answer: "국내선은 IST 출도착편만 고르고 SAW는 섞지 않습니다. 2027년 실제 시간표에서 늦은 오전 출발과 오전 또는 정오 복귀가 없으면 Option B를 중단하는 편이 낫습니다."
  },
  {
...
export function makeIcs(days = itinerary) {
  const events = list(days).map((day) => [
    "BEGIN:VEVENT",
    `UID:${day.date}-antalya@family-trip.local`,
  ...
  return ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Antalya Family Trip 2027//KO", "CALSCALE:GREGORIAN", events, "END:VCALENDAR", ""].join("\r\n");
}
