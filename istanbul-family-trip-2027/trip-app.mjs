...
export function bootTripApp({ data, core }) {
  ...
  function renderLodgingDetail() {
    ...
    const reviewEvidence = item.reviews ? `<details class="hotel-review-evidence"><summary>실제 후기에서 반복된 말</summary><div><strong>반복된 장점</strong><ul>${item.reviews.liked.map((point) => `<li>${escapeHtml(point)}</li>`).join("")}</ul><strong>반복된 불편</strong><ul>${item.reviews.disliked.map((point) => `<li>${escapeHtml(point)}</li>`).join("")}</ul><p>${escapeHtml(item.reviews.summary)}</p><div class="review-sources">${item.reviews.sources.map((source) => `<a href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.platform)}</a>`).join("")}</div></div></details>` : "";
  ...
  function renderHotelCatalog() {
    ...
    $("#hotel-catalog-grid").innerHTML = hotels.map((item) => {
      ...
      return `<article class="hotel-catalog-card ${item.id === state.lodging ? "selected" : ""}"><figure class="photo-frame"><img src="${escapeHtml(item.image)}" data-fallback="${escapeHtml(item.imageFallback || heroImage)}" alt="${escapeHtml(item.photoLabel)}" loading="lazy" decoding="async"><figcaption><a href="${escapeHtml(item.photoSource)}" target="_blank" rel="noreferrer">정확한 숙소 사진 출처</a></figcaption></figure><div class="hotel-catalog-copy"><div class="hotel-card-meta"><span>${item.rank}순위</span><span>${escapeHtml(item.location)}</span></div><h4>${escapeHtml(item.name)}</h4><p>${escapeHtml(item.verdict)}</p><div class="hotel-card-prices"><div><small>Trip.com 참고, 1실 1박</small><strong>${escapeHtml(tripPrice)}</strong></div><div><small>${escapeHtml(directLabel)}</small><strong>${escapeHtml(directPrice)}</strong></div></div><small class="hotel-observed">관측일 ${escapeHtml(quote?.capturedAt || CHECKED_AT)} / 조건이 같을 때만 차액 계산</small><div class="hotel-card-actions"><button type="button" data-hotel-select="${escapeHtml(item.id)}">가족 판단 보기</button>${quote ? `<a href="${escapeHtml(quote.sourceUrl)}" target="_blank" rel="noreferrer">Trip.com</a><a href="${escapeHtml(quote.officialDirect.sourceUrl)}" target="_blank" rel="noreferrer">공식 예약</a>` : `<a href="${escapeHtml(item.official)}" target="_blank" rel="noreferrer">공식 예약</a>`}</div></div></article>`;
    }).join("");
  ...
  function renderTripComCosts() {
    ...
    const rows = observedTripComQuotes.map((quote) => {
      ...
      return `<details class="hotel-price-row"><summary><span class="price-hotel-name">${escapeHtml(lodgingName(quote.lodgingId))}</span><span><small>Trip.com 참고, 1실 1박</small><strong>${escapeHtml(quote.nightlyDisplay)}</strong></span><span><small>${escapeHtml(directSummaryLabel(direct))}</small><strong>${escapeHtml(direct.nightlyDisplay)}</strong></span><span><small>공식 관측 상태</small><strong>${escapeHtml(statusLabel(direct.status))}</strong></span></summary><div class="hotel-price-row-body"><div class="quote-price-grid"><section class="quote-price"><small>TRIP.COM 참고 / ${escapeHtml(quote.unitLabel)}</small><strong>${escapeHtml(quote.nightlyDisplay)}</strong><p><b>${escapeHtml(quote.projectedDisplay)}</b><br>${escapeHtml(quote.stayLabel)}</p><span>${escapeHtml(quote.referenceStay)}<br>${escapeHtml(quote.occupancy)}</span><div class="price-conditions"><i>${escapeHtml(conditionLabel(quote.totalIncludesTaxes, "세금 포함", "세금 별도", "세금 미확인"))}</i><i>${escapeHtml(conditionLabel(quote.breakfast, "조식 포함", "조식 불포함", "조식 미확인"))}</i><i>${escapeHtml(conditionLabel(quote.refundable, "환불 가능", "환불 불가", "환불 조건 미확인"))}</i></div></section><section class="quote-price direct"><small>호텔 공식 / ${escapeHtml(direct.unitLabel)}</small><strong>${escapeHtml(direct.nightlyDisplay)}</strong>${direct.nightlyKrwDisplay ? `<em>${escapeHtml(direct.nightlyKrwDisplay)}</em>` : ""}<p><b>${escapeHtml(direct.projectedDisplay)}</b>${direct.projectedKrwDisplay ? ` / ${escapeHtml(direct.projectedKrwDisplay)}` : ""}<br>${escapeHtml(direct.stayLabel)}</p><span>${escapeHtml(direct.referenceStay)}<br>${escapeHtml(direct.occupancy)}</span><div class="price-conditions"><i>${escapeHtml(conditionLabel(direct.totalIncludesTaxes, "세금 포함", "세금 별도", "세금 미확인"))}</i><i>${escapeHtml(conditionLabel(direct.breakfast, "조식 포함", "조식 불포함", "조식 미확인"))}</i><i>${escapeHtml(conditionLabel(direct.refundable, "환불 가능", "환불 불가", "환불 조건 미확인"))}</i></div>${member}${alternatives}</section></div><div class="quote-comparison ${escapeHtml(comparison.status)}"><b>${escapeHtml(comparison.label)}</b><span>${escapeHtml(comparison.reason)}</span></div><div class="quote-evidence"><span>${escapeHtml(statusLabel(direct.status))}</span><span>Trip.com 관측일 ${escapeHtml(quote.capturedAt)}</span><span>공식가 관측일 ${escapeHtml(direct.capturedAt)}</span></div><p class="quote-note"><b>Trip.com:</b> ${escapeHtml(quote.inventoryNote)}<br><b>공식:</b> ${escapeHtml(direct.inventoryNote)}</p><div class="quote-links"><a href="${escapeHtml(quote.sourceUrl)}" target="_blank" rel="noreferrer">Trip.com에서 다시 확인</a><a href="${escapeHtml(direct.sourceUrl)}" target="_blank" rel="noreferrer">호텔 공식 사이트에서 확인</a></div></div></details>`;
    }).join("");
    root.innerHTML = `<article class="tripcom-cost-card benchmark"><span class="quote-status">시장 평균 비교선</span><h3>${escapeHtml(tripComCostSummary.benchmarkLabel)}</h3><div class="benchmark-price"><small>객실 1실, 1박</small><strong>${escapeHtml(tripComCostSummary.benchmarkNightly)}</strong></div><p><b>가족 4실, 10박</b> ${escapeHtml(tripComCostSummary.benchmarkTotal)}<br>${escapeHtml(tripComCostSummary.benchmarkFormula)}</p><small>${escapeHtml(tripComCostSummary.exactQuoteStatus)}</small><a href="${escapeHtml(tripComCostSummary.sourceUrl)}" target="_blank" rel="noreferrer">Trip.com 평균 원문</a></article><article class="tripcom-cost-card fx-card"><span class="quote-status">환율 기준</span><h3>${escapeHtml(tripComCostSummary.fx.label)}</h3><div class="benchmark-price"><small>EUR 1</small><strong>${escapeHtml(tripComCostSummary.fx.eurToKrw)}원</strong></div><p>${escapeHtml(tripComCostSummary.fx.note)}</p><a href="${escapeHtml(tripComCostSummary.fx.sourceUrl)}" target="_blank" rel="noreferrer">ECB 환율 원문</a></article><section class="hotel-price-list"><div class="hotel-price-list-head"><h4>호텔 30곳 가격표</h4><p>행을 열면 4실 10박 환산, 세금, 조식, 환불, 관측일과 예약 링크를 확인할 수 있습니다.</p></div>${rows}</section>`;
  }
