...
export function bootTripApp({ data, core }) {
  ...
  function renderTripComCosts() {
    ...
    const statusLabel = (status) => status === "observed_exact" ? "2027 공식 실가" : status === "observed_once_not_reproduced" ? "1회 관측, 재현 실패" : status === "verification_blocked" ? "보안 확인으로 미관측" : "공식가 미노출";
    root.innerHTML = `<article class="tripcom-cost-card benchmark"><span class="quote-status">시장 평균 비교선</span><h3>${escapeHtml(tripComCostSummary.benchmarkLabel)}</h3><div class="benchmark-price"><small>객실 1실, 1박</small><strong>${escapeHtml(tripComCostSummary.benchmarkNightly)}</strong></div><p><b>가족 4실, 10박</b> ${escapeHtml(tripComCostSummary.benchmarkTotal)}<br>${escapeHtml(tripComCostSummary.benchmarkFormula)}</p><small>${escapeHtml(tripComCostSummary.exactQuoteStatus)}</small><a href="${escapeHtml(tripComCostSummary.sourceUrl)}" target="_blank" rel="noreferrer">Trip.com 평균 원문</a></article>${observedTripComQuotes.map((quote) => {
      ...
      return `<article class="tripcom-cost-card hotel-quote"><div class="quote-card-head"><span class="quote-status warning">Trip.com 참고가</span><span>관측일 ${escapeHtml(quote.capturedAt)}</span></div><h3>${escapeHtml(lodgingName(quote.lodgingId))}</h3><div class="quote-price-grid"><section class="quote-price"><small>TRIP.COM / ${escapeHtml(quote.unitLabel)}</small><strong>${escapeHtml(quote.nightlyDisplay)}</strong><p><b>${escapeHtml(quote.projectedDisplay)}</b><br>${escapeHtml(quote.stayLabel)}</p><span>${escapeHtml(quote.referenceStay)}<br>${escapeHtml(quote.occupancy)}</span></section><section class="quote-price direct"><small>호텔 공식 / ${escapeHtml(direct.unitLabel)}</small><strong>${escapeHtml(direct.nightlyDisplay)}</strong>${direct.nightlyKrwDisplay ? `<em>${escapeHtml(direct.nightlyKrwDisplay)}</em>` : ""}<p><b>${escapeHtml(direct.projectedDisplay)}</b>${direct.projectedKrwDisplay ? ` / ${escapeHtml(direct.projectedKrwDisplay)}` : ""}<br>${escapeHtml(direct.stayLabel)}</p><span>${escapeHtml(direct.referenceStay)}<br>${escapeHtml(direct.occupancy)}</span>${member}${alternatives}</section></div><div class="quote-comparison ${escapeHtml(comparison.status)}"><b>${escapeHtml(comparison.label)}</b><span>${escapeHtml(comparison.reason)}</span></div><div class="quote-evidence"><span>${escapeHtml(statusLabel(direct.status))}</span><span>공식가 관측일 ${escapeHtml(direct.capturedAt)}</span><span>${direct.totalIncludesTaxes === true ? "세금과 수수료 포함" : "세금 포함 조건 재확인"}</span></div><p class="quote-note"><b>Trip.com:</b> ${escapeHtml(quote.inventoryNote)}<br><b>공식:</b> ${escapeHtml(direct.inventoryNote)}</p><div class="quote-links"><a href="${escapeHtml(quote.sourceUrl)}" target="_blank" rel="noreferrer">Trip.com에서 다시 확인</a><a href="${escapeHtml(direct.sourceUrl)}" target="_blank" rel="noreferrer">호텔 공식 사이트에서 확인</a></div></article>`;
    }).join("")}<article class="tripcom-cost-card fx-card"><span class="quote-status">환율 기준</span><h3>${escapeHtml(tripComCostSummary.fx.label)}</h3><div class="benchmark-price"><small>EUR 1</small><strong>${escapeHtml(tripComCostSummary.fx.eurToKrw)}원</strong></div><p>${escapeHtml(tripComCostSummary.fx.note)}</p><a href="${escapeHtml(tripComCostSummary.fx.sourceUrl)}" target="_blank" rel="noreferrer">ECB 환율 원문</a></article>`;
  }
