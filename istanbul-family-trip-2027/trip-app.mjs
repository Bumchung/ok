...
export function bootTripApp({ data, core }) {
  ...
  function renderBudget() {
    ...
    const flightLabel = result.flightPerPerson > 0 ? `항공 ${formatKrw(result.flightPerPerson)}` : "항공 실가격 미확정";
    $("#budget-result").innerHTML = `<span>${escapeHtml(result.origin.label)} 예상</span><strong>${formatKrw(result.perPersonTotal)}</strong><p>${flightLabel} + ${escapeHtml(trip.budgetStayLabel || `현지 ${trip.nights}박`)} ${formatKrw(result.landPerPerson)}</p><small>${escapeHtml(trip.budgetGroupLabel || "두 출발팀 9명 전체")}는 ${formatKrw(result.mixedOriginFamilyTotal)}, 회계상 1인 평균 ${formatKrw(result.mixedOriginAveragePerPerson)}입니다.</small>`;
  ...
  function renderFx() {
    ...
    const currencyCode = fxStrategy.currencyCode || "TRY";
    const currencyName = fxStrategy.currencyName || "현지 통화";
    $("#fx-diagnosis").textContent = fxStrategy.diagnosis;
    $("#fx-rate-board").innerHTML = `<article><span>현재 참고환율</span><strong>1 ${escapeHtml(currencyCode)} = ${fxStrategy.rates.tryKrw.toFixed(2)}원</strong><small>${escapeHtml(fxStrategy.sourceDate)} 확인</small></article><article><span>기준시점 대비 명목</span><strong>${Math.abs(fxStrategy.rates.nominalChangeSinceYearEndPct).toFixed(2)}%</strong><small>원화 기준 ${escapeHtml(currencyName)} 가격</small></article><article class="reality"><span>상방 스트레스</span><strong>${fxStrategy.rates.combinedCostChangePct.toFixed(2)}%</strong><small>확정 예측이 아닌 반증 계산</small></article>`;
  ...
  function renderHotelCatalog() {
    ...
    $("#hotel-catalog-grid").innerHTML = hotels.map((item) => {
      ...
      const referenceLabel = quote?.status === "reference_start_price" ? `${referenceProvider}, ${quote.unitLabel || "비목표일 1실 1박"}` : `${referenceProvider}, ${quote.unitLabel || "참고 1실 1박"}`;
  ...
  function renderTripComCosts() {
    ...
    const rows = observedTripComQuotes.map((quote) => {
      ...
      const referenceSummaryLabel = `${quote.provider}, ${quote.unitLabel || (quote.status === "reference_start_price" ? "비목표일 1실 1박" : "목표일 1실 1박")}`;
    ...
    const fxCode = tripComCostSummary.fx.currencyCode || "EUR";
    const fxValue = tripComCostSummary.fx.toKrw ?? tripComCostSummary.fx.eurToKrw ?? "재확인";
    root.innerHTML = `<article class="tripcom-cost-card benchmark"><span class="quote-status">시장 평균 비교선</span><h3>${escapeHtml(tripComCostSummary.benchmarkLabel)}</h3><div class="benchmark-price"><small>객실 1실, 1박</small><strong>${escapeHtml(tripComCostSummary.benchmarkNightly)}</strong></div><p><b>가족 ${roomCount}실, ${trip.nights}박</b> ${escapeHtml(tripComCostSummary.benchmarkTotal)}<br>${escapeHtml(tripComCostSummary.benchmarkFormula)}</p><small>${escapeHtml(tripComCostSummary.exactQuoteStatus)}</small><a href="${escapeHtml(tripComCostSummary.sourceUrl)}" target="_blank" rel="noreferrer">${escapeHtml(tripComCostSummary.provider)}에서 확인</a></article><article class="tripcom-cost-card fx-card"><span class="quote-status">환율 기준</span><h3>${escapeHtml(tripComCostSummary.fx.label)}</h3><div class="benchmark-price"><small>${escapeHtml(fxCode)} 1</small><strong>${escapeHtml(fxValue)}원</strong></div><p>${escapeHtml(tripComCostSummary.fx.note)}</p><a href="${escapeHtml(tripComCostSummary.fx.sourceUrl)}" target="_blank" rel="noreferrer">환율 원문</a></article><section class="hotel-price-list"><div class="hotel-price-list-head"><h4>호텔 ${observedTripComQuotes.length}곳 가격표</h4><p>행을 열면 ${roomCount}실 ${trip.nights}박 단순 환산, 세금 여부, 관측일과 실제 예약 링크를 확인할 수 있습니다.</p></div>${rows}</section>`;
  }
