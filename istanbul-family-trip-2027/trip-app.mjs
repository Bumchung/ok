...
export function bootTripApp({ data, core }) {
  ...
  function renderTripComCosts() {
    ...
    root.innerHTML = `<article class="tripcom-cost-card benchmark"><span class="quote-status">시장 평균 비교선</span><h3>${escapeHtml(tripComCostSummary.benchmarkLabel)}</h3><div class="benchmark-price"><small>객실 1실, 1박</small><strong>${escapeHtml(tripComCostSummary.benchmarkNightly)}</strong></div><p><b>가족 ${roomCount}실, ${trip.nights}박</b> ${escapeHtml(tripComCostSummary.benchmarkTotal)}<br>${escapeHtml(tripComCostSummary.benchmarkFormula)}</p><small>${escapeHtml(tripComCostSummary.exactQuoteStatus)}</small><a href="${escapeHtml(tripComCostSummary.sourceUrl)}" target="_blank" rel="noreferrer">Trip.com에서 확인</a></article><article class="tripcom-cost-card fx-card"><span class="quote-status">환율 기준</span><h3>${escapeHtml(tripComCostSummary.fx.label)}</h3><div class="benchmark-price"><small>EUR 1</small><strong>${escapeHtml(tripComCostSummary.fx.eurToKrw)}원</strong></div><p>${escapeHtml(tripComCostSummary.fx.note)}</p><a href="${escapeHtml(tripComCostSummary.fx.sourceUrl)}" target="_blank" rel="noreferrer">ECB 환율 원문</a></article><section class="hotel-price-list"><div class="hotel-price-list-head"><h4>호텔 ${observedTripComQuotes.length}곳 가격표</h4><p>행을 열면 ${roomCount}실 ${trip.nights}박 환산, 세금, 조식, 환불, 관측일과 예약 링크를 확인할 수 있습니다.</p></div>${rows}</section>`;
  }
