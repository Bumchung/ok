const PAGE_URL = "https://bumchung.github.io/ok/istanbul-family-trip-2027/";
const GATEWAY_BASE_URL = "https://ai-gateway.vercel.sh";
const GPT_MODEL = process.env.ISTANBUL_GPT_MODEL || "openai/gpt-5.4-mini";
const CLAUDE_MODEL = process.env.ISTANBUL_CLAUDE_MODEL || "anthropic/claude-haiku-4.5";
const PROVIDER_TIMEOUT_MS = Number(process.env.ISTANBUL_PROVIDER_TIMEOUT_MS || 45000);
const RATE_WINDOW_MS = 10 * 60 * 1000;
const RATE_LIMIT = Number(process.env.ISTANBUL_ASSISTANT_RATE_LIMIT || 20);
const ALLOWED_ORIGINS = new Set([
  "https://bumchung.github.io",
  "https://istanbul-family-trip-assistant.vercel.app"
]);
...
답변 규칙:
- 2027년 3월 이스탄불을 여행하는 성인 6명과 만 9세, 7세, 6세 어린이 가족의 현장 가이드처럼 답한다.
- 일행은 ICN과 LAX에서 따로 출발하며, 아이들이 지치지 않게 짧게 이동하고 오후에는 숙소로 돌아오는 여행을 원한다.
- 결론과 추천을 첫 문장에 둔다.
  ...
  const response = await fetchWithTimeout(`${GATEWAY_BASE_URL}/v1/messages`, {
    ...
    body: JSON.stringify({
      ...
      tools: [{
        ...
        name: "web_search",
        max_uses: 2,
        user_location: { type: "approximate", city: "Istanbul", region: "Istanbul", country: "TR", timezone: "Europe/Istanbul" }
      }],
