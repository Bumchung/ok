...
export function weatherMode(now = new Date()) {
  ...
  if (untilArrival > 16) {
    return { live: false, reason: trip.coreText.weatherBefore, untilArrival };
  }
  if (untilArrival < -7) {
    return { live: false, reason: trip.coreText.weatherAfter, untilArrival };
  }
...
function searchableRecords(days = itinerary) {
  ...
  const budgetRecords = list(budgetModel?.stayOptions).map((item) => {
    ...
    return {
      ...
      body: `${item.note} ${trip.destination} 3박 현지 가족 총액 ${Math.round(result.landFamilyTotal).toLocaleString("ko-KR")}원, 항공 계획값 포함 전체 참고액 ${Math.round(result.mixedOriginFamilyTotal).toLocaleString("ko-KR")}원. ${trip.coreText.budget}`,
      url: "#budget"
    };
  });
  ...
  const fxRecord = fxStrategy ? [{
    ...
    subtitle: Number.isFinite(fxStrategy.rates?.tryKrw) ? `1 ${fxStrategy.currencyCode || trip.fxCurrencyCode} ${fxStrategy.rates.tryKrw.toFixed(2)}원` : "환율 재확인 필요",
...
function hotelAnswer() {
  ...
  return `우선 확인할 호텔은 ${names}입니다. 객실 4실의 연결 또는 인접 배정, 실제 침대 9개와 어린이 시설 규칙을 서면으로 받은 뒤 결정하세요. ${trip.arrivalDate}부터 ${trip.checkoutDate}까지의 같은 조건 실가격은 아직 확정값으로 쓰지 않습니다.`;
}
...
function residenceAnswer() {
  ...
  return `레지던스 후보 ${candidates.length}곳 중 먼저 볼 곳은 ${names || "아직 없습니다"}입니다. 현재 카드는 구조와 권역을 비교하는 후보 목록이며 2027년 가격, 관광 임대 허가, 실제 침대 9개, 욕실 수와 어린이 안전장치는 예약 전에 다시 확인해야 합니다.`;
}

function budgetAnswer() {
  const result = calculateBudget({ stayId: budgetModel.defaultStay, originId: budgetModel.defaultOrigin });
  return `${trip.coreText.budget} 기본 시나리오의 가족 참고액은 예비비 포함 약 ${Math.round(result.mixedOriginFamilyTotal / 10_000).toLocaleString("ko-KR")}만원입니다. 실제 2027년 항공, 객실과 운항 조건이 열리면 같은 조건으로 다시 계산하세요.`;
}

const questionRules = [
  {
    terms: ["비오", "비가", "우천", "날씨", "모래바람"],
    answer: trip.coreText.rain
  },
  {
    terms: ["박물관", "museum"],
    answer: trip.coreText.museum
  },
  {
    terms: trip.coreText.specialTerms,
    answer: trip.coreText.specialAnswer
  },
  {
  ...
  {
    terms: ["항공", "비행", "공항", "직항", "icn", "lax", "ist", "dxb", "doh"],
    answer: trip.coreText.transport
  },
  {
...
export function makeIcs(days = itinerary) {
  const events = list(days).map((day) => [
    "BEGIN:VEVENT",
    `UID:${day.date}-${trip.coreText.uid}@family-trip.local`,
  ...
  return ["BEGIN:VCALENDAR", "VERSION:2.0", `PRODID:-//${trip.coreText.prodid}//KO`, "CALSCALE:GREGORIAN", events, "END:VCALENDAR", ""].join("\r\n");
}
