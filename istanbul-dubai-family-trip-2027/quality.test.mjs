...
test("visible comparison states the fixed Istanbul and final nonstop split", async () => {
  const html = await readFile(join(here, "index.html"), "utf8");
  for (const phrase of [
    "IST 7박 → DXB 3박",
    "달력상 여행은 10박",
  ...
  assert.match(html, /₩933,200부터/);
  assert.match(html, /추천 다구간과는 다른 상품/);
});
...
test("entry point reuses the shared renderer with Dubai data and core", async () => {
...
test("Dubai action colors retain readable text and visible control boundaries", async () => {
  ...
  assert.ok(contrast("#718b89", "#fffaf5") >= 3, "inactive control border contrast");
...
test("map, nearby, assistant and export controls keep their accessible labels", async () => {
  const html = await readFile(join(here, "index.html"), "utf8");
  assert.match(html, /id="map-canvas"[^>]+aria-label="두바이 장소 지도"/);
...
test("assistant-authored Dubai files contain no Unicode middle dot", async () => {
